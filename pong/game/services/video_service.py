"""
A module that contains the Video Service class
"""
import pyray
import constants
...
class VideoService:
    ...
    def draw_rectangle(self, x, y, width, height, color):
        """
        Draws a rectangle on the screen (for the line down the middle and paddles)

        """
        pyray.draw_rectangle(x, y, width, height, color.to_tuple())

    def draw_circle(self, pos, radius, color):
        """ 
        Draws a circle (for the ball)
        """
        pyray.draw_circle(int(pos.get_x()), int(
            pos.get_y()), radius, color.to_tuple())
    ...
    def _get_x_offset(self, text, font_size):
        """
        Puts the font in the middle of the screen
        """
        width = pyray.measure_text(text, font_size)
        return int(width / 2)
